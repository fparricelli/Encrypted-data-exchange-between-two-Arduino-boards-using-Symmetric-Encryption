 ...
 * Useremo un ReceiverHelper per ricevere uno (o pi�) messaggi da un sender; la classe
 * si occuper� di gestire anche tutte le problematiche legate al download dei certificati
 * e alla cifratura.
 * 
 * Lato Ricevente supponiamo quindi di avere due keystore:
 * 		
 * 		> RSA SOFTWARE KEYSTORE: contiene la chiave privata RSA.
 * 	 	La corrispondente chiave pubblica RSA sar� contenuta nel certificato del ricevente.
 * 
 * 		> AES HARDWARE KEYSTORE: conterr� la chiave segreta (AES) che useremo per la cifratura dei
 * 		messaggi.
 * 		Da notare che il ricevente deve prima ricevere la chiave AES dal mittente (in fase di handshake),
 * 		dopodich� tale chiave verr� iniettata all'interno dell'hardware keystore e usata per la cifratura
 * 		di tutti i messaggi.
 * 
 * Quindi lo scambio della chiave AES verr� gestito tramite crittografia asimmetrica (RSA),
 * mentre la ricezione del messaggio sar� gestita mediante crittografia simmetrica (AES).
 * 
 * La dinamica � la seguente: la prima cosa da fare � avviare l'Handshake, che si occuper�
 * innanzitutto di attendere l'arrivo delle informazioni da parte del mittente: quest'ultimo
 * invier� la chiave AES (cifrata con la chiave pubblica del ricevente) e la firma della chiave AES
 * (ottenuta usando la chiave privata del mittente), insieme all'identit� del mittente.
 * Con tali informazioni andiamo innanzitutto a recuperare il certificato del mittente (scaricandolo da un server
 * ad-hoc, a meno che non sia gi� presente sul file-system).
 ...
 * dopodich� sfrutteremo il certificato del mittente per ricavare la sua chiave pubblica e 
 * verificare la firma della chiave AES.
 * Al termine di queste attivit�, invieremo un ACK al mittente, che indica il concludersi dell'handshake.
 * Dopodich� ci metteremo in attesa di ricevere il messaggio cifrato con la chiave AES da parte del mittente; una volta
...
public class ReceiverHelper{
	...
	//Hardware Keystore di tipo AES, conterr� la chiave AES usata per decifrare i messaggi
	private MyAESKeystore AESKeystore;
	
	//URL al quale � possibile scaricare il certificato del mittente, se necessario
	private final String senderCertDownloadURL = "http://localhost:8080/CertificateServer/getCertificate?id=";
	
	//Path al quale verr� salvato il certificato del mittente, se necessario - Decomment for Windows
	...
	//Socket che viene associata al client con cui � in corso la comunicazione corrente
	 ...
	 * Se il certificato � gi� presente sul filesystem (al path defaultDownloadURL+id) e non � scaduto,
	...
	private void initCertificate(String senderName) throws Exception{
		...
		if(f.exists()) {
			...
			System.out.println("[Ricevente - Handshake] Certificato mittente gi� presente sul file system!");
	...
	/* Procedura che d� il via all'handshake lato ricevente.
	 * Si occupa di accettare eventuali richieste di connessione in entrata (assegnando la variabile membro
	 * clientSocket).
	 * In pi�, aspetta di ricevere dal mittente l'oggetto SecureKey contenente chiave AES cifrata
	...
	public void startHandshake() throws Exception {
		...
		//Leggo l'oggetto SecureKey inviatomi dal client mittente, in cui sar� contenuta
		//l'identit� del mittente (necessaria per ricavarne il certificato), la firma e la cifratura della chiave AES.
		...
		//altrimenti...
			...
			System.out.println("[Ricevente - Handshake] La verifica � andata a buon fine.");
			...
			//Se la verifica � andata a buon fine, posso fidarmi della chiave AES che ho ricevuto
			//E iniettarla nel mio hardware keystore AES, in modo da usarla per le successive
			//comunicazioni con quel mittente
			
			/* Commento perché cambiata l'API*/
	 ...
	 * La firma che viene inviata dal mittente � stata ottenuta usando la chiave privata (del mittente),
	 ...
	 * dell'handshake.
	 * Si occupa di attendere l'arrivo del messaggio (cifrato con la chiave AES) dal mittente, dopodich�
	 * una volta ricevuto viene inviato l'ACK al mittente e si procede a decifrare il messaggio.
	 */
	public void receiveMessage() throws Exception {
		
		//Se la socket di comunicazione non � attiva..
		...
		/* Commento perchè è cambiata l'API */
	...
	/* Procedura utilizzata per chiudere la socket associata al client con cui � attiva la 
	 * comunicazione corrente.
	 * Da notare che una volta che � stato effettuato l'handshake, per ricevere messaggi da un determinato
