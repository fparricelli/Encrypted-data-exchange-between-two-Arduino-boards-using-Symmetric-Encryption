 ...
 * Useremo un SenderHelper per inviare uno (o piï¿½) messaggi al
 * ricevente; questa classe si occuperï¿½ anche di gestire gli aspetti legati
 ...
 * 	 	La corrispondente chiave pubblica RSA sarï¿½ contenuta nel certificato del sender.
 * 
 ...
 * mentre l'invio del messaggio sarï¿½ gestito mediante crittografia simmetrica (AES).
 * 
 * 
 * La dinamica del protocollo ï¿½ la seguente: lato sender innanzitutto eseguiremo la procedura
 ...
 * lo scambio della chiave AES in maniera sicura, dopodichï¿½ attenderemo un ACK da quest'ultimo.
 * All'ottenimento del messaggio di ACK, possiamo procedere con l'invio del messaggio vero e proprio:
 * tale messaggio verrï¿½ prima cifrato usando la chiave AES (precedentemente scambiata e concordata), 
...
public class SenderHelper{
	...
	//URL al quale ï¿½ possibile scaricare il certificato del ricevente, se necessario
	...
	//Stringa che indica l'identitï¿½ del mittente
	 ...
	 * Tale firma verrï¿½ ottenuta usando la chiave privata contenuta nel keystore software di tipo RSA,
	 * mediante l'algoritmo di hashing md5.
	 * La firma cosï¿½ ottenuta viene utilizzata per settare la variabile membro SecureKey.
	 */ 
	 ...
	 * Se il certificato ï¿½ giï¿½ presente sul filesystem (al path certificatePath, variabile membro) e non ï¿½ scaduto,
	 * allora viene prelevato e utilizzato per inizializzare la variabile membro corrispondente,
	 * Se ciï¿½ non accade, allora il certificato deve essere scaricato chiamando la procedura downloadCertificate(name).
	 */
	 ...
	 * chiamando la funzione getIDByName, si ricaverï¿½ l'ID del ricevente a partire dal suo nome che, 
	 * concatenato al defaultURL, ci darï¿½ l'URL al quale scaricare il certificato.
	 */
	 ...
	 * ricavata dal certificato che ï¿½ stato precedentemente scaricato.
	 ...
	 * si effettuerï¿½ una lookup che restituirï¿½ hostname e port, che verranno usati per inizializzare la Socket
	 * di comunicazione con il ricevente.
	 * Chiama le procedure per inizializzare certificato, firma e cifratura.
	 * All'invocazione, l'oggetto SecureKey:secKey sarï¿½ inizializzato e possiederï¿½ al proprio interno
	 * la chiave AES cifrata e la sua firma, che verranno inviate al ricevente.
	 * Dopodichï¿½, si attende l'ACK dal ricevente per chiudere l'handshake.
	 ...
	 * Una volta chiusa la socket, per poter ricominicare la comunicazione ï¿½ necessario
	 ...
	 * hostname e port al quale questo puï¿½ essere contattato (ad esempio, al quale possiamo
	 * contattare la sua mailbox).
	 * Nel caso reale si avrï¿½ una procedura piï¿½ complessa; nel nostro caso restituiamo direttamente i dati
	 * che ci servono.
	 */ 
