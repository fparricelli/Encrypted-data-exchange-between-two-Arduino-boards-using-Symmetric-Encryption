...
public class MessagingHelper {
	...
	public boolean sendMessage(String sender, int destinationPort, String msg, ChatFrame cf) {
		...
		try {
			...
			//Controllo se ci sono chat gi� attive con l'interlocutore richiesto (destinationPort)
			...
			if(ac==null) {
				...
				//Creo la socket che verr� usata per la comunicazione con l'interlocutore
				...
				//Dopodich� inizializzo gli stream della socket, settando le variabili membro specifiche
				...
				//Che si occuper� di ricevere i messaggi dall'interlocutore 
				...
				//Ho gi� una chat attiva con l'interlocutore, quindi evito l'inizializzazione
			...
			//Creo l'oggetto messaggio da inviare, passando la mia identit� (sender string e mia porta di origine) insieme
	...
	public void stopUpdates(int destPort) {
		...
		if(ac!=null) {
			...
			//Dopodich� rimuovo l'active chat dalla lista delle chat attive, poich� lo stopUpdates()
			//� l'ultimo metodo che viene chiamato in caso di chiusura della chat
			//e di conseguenza � responsabile di 'chiudere la porta' rimuovendo la entry dell'arraylist
			chats.remove(ac);
		}else {
